...
export interface GlorifierSummaryReport {
  ...
  assets: { accountCount: number; holdingCount: number; byClass: Record<string, number>; marketValueByCurrency: Record<string, number>; marketValueKnownCount: number; sourceBackedHoldingCount: number; verification: { verified: number; notVerified: number }; };
...
export async function buildGlorifierSummaryReport(): Promise<GlorifierSummaryReport> {
  ...
  const holdings = await db.query("SELECT COUNT(*)::int AS count, COUNT(*) FILTER (WHERE market_value IS NOT NULL)::int AS known_value, COUNT(*) FILTER (WHERE source IS NOT NULL AND source <> '')::int AS source_backed, COUNT(*) FILTER (WHERE verification_status='verified')::int AS verified, COUNT(*) FILTER (WHERE COALESCE(verification_status,'not_verified') <> 'verified')::int AS not_verified FROM asset_holdings");
  ...
  return {
    ...
    assets: { accountCount: Number(accounts.rows[0]?.count || 0), holdingCount: Number(holdings.rows[0]?.count || 0), byClass, marketValueByCurrency, marketValueKnownCount: Number(holdings.rows[0]?.known_value || 0), sourceBackedHoldingCount: Number(holdings.rows[0]?.source_backed || 0), verification: { verified: Number(holdings.rows[0]?.verified || 0), notVerified: Number(holdings.rows[0]?.not_verified || 0) } },
    ...
    claimable: { status: 'not_recorded', count: 0, estimatedValueByCurrency: {}, verifiedValueByCurrency: {}, note: 'No claimable amount is inferred from asset or market data. Any future calculated claimable amount remains NOT VERIFIED until qualifying evidence is recorded.' },
