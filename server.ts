...
const requireAuth = (req: Request, res: Response, next: Function) => {
  if (!authConfigured) return res.status(503).json({ error: 'Auth0 is not configured' });
...
function getGenAI(): GoogleGenAI | null {
  if (!genAIClient && process.env.GEMINI_API_KEY) genAIClient = new GoogleGenAI({ apiKey: process.env.GEMINI_API_KEY });
  return genAIClient;
}

app.get('/api/ai/providers', (_req: Request, res: Response) => res.json({ providers: aiOrchestrator.registry() }));
...
app.post('/api/ai/orchestrate', async (req: Request, res: Response) => {
  try {
    ...
    return res.json(await aiOrchestrator.generate({ messages, provider, model, temperature, maxTokens }));
  } catch (error) {
...
app.post('/api/ai/collaborate', async (req: Request, res: Response) => {
  try {
    ...
    if (!Array.isArray(messages) || messages.length === 0) return res.status(400).json({ error: 'messages must be a non-empty array' });
    return res.json({ results: await aiOrchestrator.collaborate(messages, providers) });
  } catch (error) {
...
app.get('/api/health', (_req: Request, res: Response) => {
  res.json({ status: 'ok', timestamp: new Date().toISOString(), auth0Configured });
});
...
Data streams summary: ${footprintsSummary || 'Browsing, E-Commerce, Developer, Health telemetry active'}.`;
    ...
    if (lower.includes('clawback') || lower.includes('delete') || lower.includes('broker')) suggestedAction = { label: 'Dispatch Statutory Erasure Notices', type: 'opt_out_all' };
    else if (lower.includes('earn') || lower.includes('more') || lower.includes('maximize') || lower.includes('yield')) suggestedAction = { label: 'Optimize Yield Curve', type: 'maximize_yield' };
    else if (lower.includes('privacy') || lower.includes('shield') || lower.includes('strict')) suggestedAction = { label: 'Tighten Differential Privacy', type: 'apply_policy' };
    ...
    const ai = getGenAI();
    if (!ai) return res.json({ reply: '[Broker Local Engine]: No connected AI provider is configured. Add a provider key in the server environment.', suggestedAction: { label: 'Configure an AI Provider', type: 'configure_provider' } });
...
app.post('/api/ai/evaluate-offer', async (req: Request, res: Response) => {
  try {
    ...
    const result = await aiOrchestrator.generate({
      messages: [{ role: 'user', content: `Evaluate this data purchase offer. Return JSON with score (0-100), verdict (RECOMMEND|CAUTION|REJECT), reasoning, and optional suggestedCounterUsd.
...
User floor: $${userPolicy?.minimumMonthlyFloorUsd}; epsilon: ${userPolicy?.globalEpsilon}` }],
    ...
    try { return res.json(JSON.parse(result.text)); }
    catch { return res.json({ reasoning: result.text, provider: result.provider, model: result.model }); }
  } catch (error) {
...
app.post('/api/ai/audit-footprint', async (req: Request, res: Response) => {
  try {
    ...
    const result = await aiOrchestrator.generate({
      messages: [{ role: 'user', content: `Perform a privacy leakage audit. Source: ${sourceName} (${category}). Sample records: ${JSON.stringify(sampleData)}. Return JSON with reidentificationRisk, recommendedEpsilon, kAnonymityMin, fairMarketMonthlyUsd, and sanitizationReport.` }],
      provider: 'auto', temperature: 0.2,
    });
    try { return res.json(JSON.parse(result.text)); }
    catch { return res.json({ sanitizationReport: result.text, provider: result.provider, model: result.model }); }
  } catch (error) {
...
app.post('/api/ai/generate-clawback', async (req: Request, res: Response) => {
  try {
    ...
    try { return res.json(JSON.parse(result.text)); }
    catch { return res.json({ documentTitle: 'Generated Data Rights Notice', legalNotice: result.text }); }
  } catch (error) {
...
async function startServer() {
  ...
  app.listen(PORT, '0.0.0.0', () => console.log(`AI Orchestrator running on http://0.0.0.0:${PORT}`));
}
